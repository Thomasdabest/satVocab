...
import { View, WordItem, User } from './types';
...
const App: React.FC = () => {
  ...
  const toggleSaveWord = (word: string) => {
    ...
    if (updatedUser.savedWords.includes(word)) {
      updatedUser.savedWords = updatedUser.savedWords.filter(w => w !== word);
    } else {
      updatedUser.savedWords = [...updatedUser.savedWords, word];
    }
    ...
    if (usersJson) {
      ...
      if (userIndex !== -1) {
        users[userIndex].savedWords = updatedUser.savedWords;
        localStorage.setItem('vocab_app_users', JSON.stringify(users));
      }
    }
    
    // Update Session
    localStorage.setItem('vocab_app_session', JSON.stringify(updatedUser));
  };
  ...
  const renderView = () => {
    const viewContent = (() => {
      switch (currentView) {
        case View.Landing:
          return <LandingView 
                    onGetStarted={() => setCurrentView(currentUser ? View.Home : View.Login)} 
        ...
        case View.Flashcards:
          return <FlashcardView 
                    ...
                    savedWords={currentUser?.savedWords || []}
                  />;
        case View.Quiz:
          return <QuizView onBack={() => setCurrentView(View.Home)} userName={currentUser?.name || "Scholar"} />;
        case View.Game:
          ...
          return (
            <div className="flex flex-col items-center justify-center min-h-[70vh] text-center px-4">
              <h1 className="text-5xl font-black text-slate-900 mb-4 tracking-tight">
                Hello, <span className="text-indigo-600 font-black">{currentUser?.name || 'Scholar'}</span>
              </h1>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full max-w-6xl">
                ...
                >
                  ...
                  <p className="text-slate-500 font-medium">Browse the full library with AI visuals and sounds.</p>
                </button>
                ...
                >
                  ...
                  <p className="text-slate-500 font-medium">Test your knowledge with randomized questions.</p>
                </button>
                ...
                >
                  ...
                  <h2 className="text-2xl font-black mb-2 text-slate-800">SAT Vocabulary Test</h2>
                  <p className="text-slate-500 font-medium">A quick 20-word session to check your mastery.</p>
                </button>
                ...
                >
                  ...
                  <p className="text-slate-500 font-medium">Focus on the {currentUser?.savedWords.length || 0} words you've saved.</p>
                </button>
              </div>
            </div>
          );
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <nav className="bg-white/80 backdrop-blur-md border-b border-slate-200 py-4 px-6 flex justify-between items-center sticky top-0 z-50">
        <div 
          ...
          onClick={() => setCurrentView(currentUser ? View.Home : View.Landing)}
        >
        ...
        <div className="flex gap-3 items-center">
          {currentUser ? (
            <>
              ...
              >
                Saved ({currentUser.savedWords.length})
              </button>
              <button 
